...
public class IdempotentParamFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (!StringUtils.hasText(globalIdempotentId)) {
            globalIdempotentId = generateGlobalIdempotentId(request, webRequestWrapper.getRequestData());
        }

        if (StringUtils.hasText(globalIdempotentId)) {
            globalIdempotentId = DigestUtils.md5DigestAsHex(globalIdempotentId.getBytes());
            ContextHolder.getCurrentContext().add(IdempotentConstants.IDEMPOTEMT_ID_NAME, globalIdempotentId);
            log.debug("全局幂等ID {}", globalIdempotentId);
        }

        filterChain.doFilter(webRequestWrapper, servletResponse);
    }
    ...
    private String getGlobalIdempotentId(HttpServletRequest request) {
        // 参数中获取
        String globalIdempotentId = request.getParameter(IdempotentConstants.IDEMPOTEMT_ID_NAME);
        if (StringUtils.hasText(globalIdempotentId)) {
            return globalIdempotentId;
        }

        // 请求头中获取
        globalIdempotentId = request.getHeader(IdempotentConstants.IDEMPOTEMT_ID_NAME);
        if (StringUtils.hasText(globalIdempotentId)) {
            return globalIdempotentId;
        }
